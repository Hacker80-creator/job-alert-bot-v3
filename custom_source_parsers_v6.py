"""Facet-aware Workday parser layered over all production custom feeds."""
from __future__ import annotations

import time
from typing import Any

import custom_source_parsers_v5 as previous
import job_match_expanded as expanded
import job_monitor as bot


BASE_CUSTOM_FETCH = previous.fetch_company_jobs_with_custom_v5


def _location_from_workday_path(path: str) -> str:
    if "/job/" not in path:
        return ""
    segment = path.split("/job/", 1)[1].split("/", 1)[0]
    return segment.replace("-", " ")


def parse_workday_faceted(company: dict[str, Any]) -> list[bot.Job]:
    """Search a Workday tenant using verified location facet IDs."""
    ...
    for term in terms:
        for offset in range(0, max_results, page_size):
            ...
            for item in raw_jobs:
                ...
                if location.casefold() in {"2 locations", "3 locations", "4 locations"}:
                    path_location = _location_from_workday_path(path)
                    if path_location:
...
def fetch_company_jobs_with_custom_v6(company: dict[str, Any]) -> list[bot.Job]:
    if company.get("ats") != "workday_faceted":
        return BASE_CUSTOM_FETCH(company)
    try:
        jobs = parse_workday_faceted(company)
